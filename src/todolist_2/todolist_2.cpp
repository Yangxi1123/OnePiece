...
void saveTasksToFile(const std::vector<std::string>& tasks, const std::string& filename) {
    std::ofstream outputFile(filename); // 创建输出文件流（打开文件准备写入）
    ...
    // 将每个任务写入文件，每行一个任务
    for (const auto& task : tasks) { // 遍历所有任务
        outputFile << task << std::endl;// 将任务写入文件，并添加换行符
    }
